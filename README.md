...
- **parse** — синтаксический анализатор (парсер) языка Mython (В учебном задании этот модуль предоставлен авторами. Его реализация требует определённой теоретической подготовки, выходящей за рамки пройденого курса по C++).
